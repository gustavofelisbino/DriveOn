...
export default function Login() {
  const { signIn, signInWithCompany, isAuthenticated } = useAuth();
  ...
  useEffect(() => {
    if (isAuthenticated) nav(paths.root, { replace: true });
  }, [isAuthenticated, nav]);
  ...
  return (
    <Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', md: '1fr 1fr' }, minHeight: '100vh', bgcolor: 'background.default' }}>
      <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'center', px: { xs: 2.5, md: 8 } }}>
        <Paper elevation={0} sx={{ width: '100%', maxWidth: 520, bgcolor: 'transparent' }}>
          ...
          <form onSubmit={empresaChoices ? handleSubmitComEmpresa : handleSubmit} noValidate>
            <TextField label="Endereço de e-mail" fullWidth value={email} onChange={(e) => setEmail(e.target.value)} sx={fieldSx} autoComplete="email" />
            <TextField
              label="Senha" fullWidth type={show ? 'text' : 'password'} value={password} onChange={(e) => setPassword(e.target.value)}
              sx={{ ...fieldSx, mb: 1.5 }} autoComplete="current-password"
            ...
            {empresaChoices && (
              <TextField select label="Empresa" fullWidth value={empresaIdSelecionada}
                onChange={(e) => setEmpresaIdSelecionada(Number(e.target.value))} sx={fieldSx}
                helperText="Seu e-mail existe em mais de uma empresa — selecione a correta.">
                {empresaChoices.map(c => (<MenuItem key={c.id} value={c.id}>{c.nome} (#{c.id})</MenuItem>))}
              </TextField>
            )}
            ...
            <Stack direction="row" alignItems="center" justifyContent="space-between" sx={{ mb: 2 }}>
              ...
              <Button variant="text" sx={{ textTransform: 'none', p: 0, minWidth: 0 }}>Esqueceu sua senha?</Button>
            </Stack>

            {error && <Typography color="error" variant="body2" sx={{ mb: 1 }}>{error}</Typography>}

            <Button type="submit" variant="contained" fullWidth size="medium" disabled={loading} sx={{ height: 50, borderRadius: 2, fontWeight: 700 }}>
      ...
      <Box sx={{ display: { xs: 'none', md: 'block' }, position: 'relative', borderLeft: (t) => `1px solid ${t.palette.divider}`, overflow: 'hidden' }}>
        <Box component="img" src={mechanic} alt="Mecânico usando celular" sx={{ width: '100%', height: '100vh', objectFit: 'cover', objectPosition: 'initial', display: 'block' }} />
      </Box>
    </Box>
  );
}
